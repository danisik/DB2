...
CREATE OR REPLACE FUNCTION VYTVOR_TRIGGER_KONTROLA_STAVU_HRY() RETURNS TRIGGER AS $VYTVOR_TRIGGER_KONTROLA_STAVU_HRY$
	DECLARE
		...
		KURZOR_TAHU CURSOR FOR SELECT * FROM TAH WHERE ID_AKTUALNI_HRA = NEW.ID_AKTUALNI_HRA;
	BEGIN				
		SELECT * INTO I_HRA FROM HRA WHERE ID = NEW.ID_AKTUALNI_HRA;
		
		
		-- Hlídání a aktualizace stavu hry, tj. zda nedospěla do remízy nebo vítězství jednoho z hráčů (volání funkcí REMIZA a VYHRA).
		-- Kontrola s využitím tabulky TAH
		-- ID, CAS_TAHU, ID_AKTUALNI_HRA, ID_HRAJICI_HRAC, X, Y
		
		IF I_HRA IS NULL THEN
			UPDATE HRA SET ID_STAV = NOVY_STAV_HRY;
		END IF;
