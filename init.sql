...
CREATE TABLE HRAC (
	...
	TAH_PRVNI_POCET_VYHER INTEGER DEFAULT 0,
	TAH_PRVNI_POCET_REMIZ INTEGER DEFAULT 0,
	TAH_PRVNI_POCET_PROHER INTEGER DEFAULT 0,
	TAH_DRUHY_POCET_VYHER INTEGER DEFAULT 0,
	TAH_DRUHY_POCET_REMIZ INTEGER DEFAULT 0,
	TAH_DRUHY_POCET_PROHER INTEGER DEFAULT 0
);
...
CREATE TABLE HRA (
	...
	ID_STAV INTEGER DEFAULT 1,	
	ZACATEK_HRY TIMESTAMP(6) DEFAULT CURRENT_TIMESTAMP,
	ID_PRVNI_HRAC INTEGER NOT NULL,
	CAS_PRVNI_HRAC INTEGER DEFAULT 0,
	ID_DRUHY_HRAC INTEGER NOT NULL,
	CAS_DRUHY_HRAC INTEGER DEFAULT 0,
...
CREATE TABLE TAH (
	ID SERIAL PRIMARY KEY,
	CAS_TAHU TIMESTAMP(6) DEFAULT CURRENT_TIMESTAMP,
...
CREATE OR REPLACE FUNCTION HERNI_CAS(ID_HRA INTEGER, ID_HRAC INTEGER) RETURNS INTEGER AS $$
	DECLARE
		...
		ZASTAVOVACI_PODMINKA INTEGER := 0;
		i_HRA HRA;		
		CURSOR_PRVNI_HRAC REFCURSOR;
		CURSOR_DRUHY_HRAC REFCURSOR;
		
		HERNI_CAS_PRVNI_HRAC TIMESTAMP(6);
		HERNI_CAS_DRUHY_HRAC TIMESTAMP(6);		
	BEGIN
		SELECT * INTO I_HRA FROM HRA WHERE ID = ID_HRA;	
		
		OPEN CURSOR_PRVNI_HRAC FOR SELECT CAS_TAHU FROM TAH WHERE ID_AKTUALNI_HRA = ID_HRA AND ID_HRAJICI_HRAC = ID_HRAC;
		
		SELECT COUNT(*) INTO POCET_CASU_PRVNI_HRAC FROM TAH WHERE ID_AKTUALNI_HRA = ID_HRA AND ID_HRAJICI_HRAC = ID_HRAC;
		
		IF ID_HRAC = I_HRA.ID_PRVNI_HRAC THEN
			OPEN CURSOR_DRUHY_HRAC FOR SELECT CAS_TAHU FROM TAH WHERE ID_AKTUALNI_HRA = ID_HRA AND ID_HRAJICI_HRAC = I_HRA.ID_DRUHY_HRAC;	
			SELECT COUNT(*) INTO POCET_CASU_DRUHY_HRAC FROM TAH WHERE ID_AKTUALNI_HRA = ID_HRA AND ID_HRAJICI_HRAC = I_HRA.ID_DRUHY_HRAC;			
		ELSE
			OPEN CURSOR_DRUHY_HRAC FOR SELECT CAS_TAHU FROM TAH WHERE ID_AKTUALNI_HRA = ID_HRA AND ID_HRAJICI_HRAC = I_HRA.ID_PRVNI_HRAC;				
			SELECT COUNT(*) INTO POCET_CASU_DRUHY_HRAC FROM TAH WHERE ID_AKTUALNI_HRA = ID_HRA AND ID_HRAJICI_HRAC = I_HRA.ID_PRVNI_HRAC;
		END IF;						
		...
		LOOP
			...
			IF ID_HRAC = I_HRA.ID_PRVNI_HRAC THEN
				IF ZASTAVOVACI_PODMINKA = 0 THEN
					FETCH CURSOR_PRVNI_HRAC INTO HERNI_CAS_PRVNI_HRAC;
					
					VYSLEDNY_CAS := VYSLEDNY_CAS + EXTRACT (EPOCH FROM (HERNI_CAS_PRVNI_HRAC::TIMESTAMP - I_HRA.ZACATEK_HRY::TIMESTAMP))::INTEGER;
				ELSE
					...
					FETCH CURSOR_PRVNI_HRAC INTO HERNI_CAS_PRVNI_HRAC;
					FETCH CURSOR_DRUHY_HRAC INTO HERNI_CAS_DRUHY_HRAC;									
										
					VYSLEDNY_CAS := VYSLEDNY_CAS + EXTRACT (EPOCH FROM (HERNI_CAS_PRVNI_HRAC::TIMESTAMP - HERNI_CAS_DRUHY_HRAC::TIMESTAMP))::INTEGER;
				END IF;
			ELSE				
				FETCH CURSOR_PRVNI_HRAC INTO HERNI_CAS_PRVNI_HRAC;
				FETCH CURSOR_DRUHY_HRAC INTO HERNI_CAS_DRUHY_HRAC;
		...
		CLOSE CURSOR_PRVNI_HRAC;
		CLOSE CURSOR_DRUHY_HRAC;
		RETURN VYSLEDNY_CAS;
...
CREATE OR REPLACE PROCEDURE STATISTIKY (ID_HRA INTEGER) AS $$
	...
	BEGIN
		...
		IF I_HRA.ID_STAV = 2 THEN
			UPDATE HRAC SET TAH_PRVNI_POCET_VYHER = TAH_PRVNI_POCET_VYHER + 1 WHERE ID = I_HRA.ID_PRVNI_HRAC;
			UPDATE HRAC SET TAH_DRUHY_POCET_PROHER = TAH_DRUHY_POCET_PROHER + 1 WHERE ID = I_HRA.ID_DRUHY_HRAC;
		ELSIF I_HRA.ID_STAV = 3 THEN
			UPDATE HRAC SET TAH_PRVNI_POCET_PROHER = TAH_PRVNI_POCET_PROHER + 1 WHERE ID = I_HRA.ID_PRVNI_HRAC;
			UPDATE HRAC SET TAH_DRUHY_POCET_VYHER = TAH_DRUHY_POCET_VYHER + 1 WHERE ID = I_HRA.ID_DRUHY_HRAC;
		ELSIF I_HRA.ID_STAV = 4 THEN
			UPDATE HRAC SET TAH_PRVNI_POCET_REMIZ = TAH_PRVNI_POCET_REMIZ + 1 WHERE ID = I_HRA.ID_PRVNI_HRAC;
			UPDATE HRAC SET TAH_DRUHY_POCET_REMIZ = TAH_DRUHY_POCET_REMIZ + 1 WHERE ID = I_HRA.ID_DRUHY_HRAC;
		END IF;
	END;
	$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION VYTVOR_TRIGGER_KONTROLA_STAVU_HRY() RETURNS TRIGGER AS $VYTVOR_TRIGGER_KONTROLA_STAVU_HRY$
	DECLARE
		...
		MAX_POCET_TAHU INTEGER;
		POCET_TAHU_V_HRE INTEGER;
	...
	BEGIN			
		...
		LOOP
			...
			LOOP
				...
				PERFORM 1 FROM TAH WHERE ID_AKTUALNI_HRA = NEW.ID_AKTUALNI_HRA AND X = AKTUALNI_X AND Y = AKTUALNI_Y AND ID_HRAJICI_HRAC = NEW.ID_HRAJICI_HRAC LIMIT 1;
		...
		IF RADA_EXISTUJE = TRUE THEN
			...
			END IF;
		ELSE
			MAX_POCET_TAHU = I_HRA.SIRKA * I_HRA.VYSKA;
			SELECT COUNT(*) INTO POCET_TAHU_V_HRE FROM TAH WHERE ID_AKTUALNI_HRA = NEW.ID_AKTUALNI_HRA;
			
			IF MAX_POCET_TAHU = POCET_TAHU_V_HRE THEN
				NOVY_STAV_HRY = 4;
			END IF;
		END IF;		
