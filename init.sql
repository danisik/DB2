...
DROP TRIGGER IF EXISTS VYTVOR_PARAMETRY ON HRA;
...
CREATE OR REPLACE FUNCTION VYTVOR_TRIGGER_HRA_DOHRANA() RETURNS TRIGGER AS $VYTVOR_TRIGGER_HRA_DOHRANA$
	DECLARE
	BEGIN
		IF NEW.ID_STAV = OLD.ID_STAV THEN
			RETURN NULL;
		END IF;
		
		--SPOCITAT HERNI DOBY HRACU (KONEC_HRY(ID_HRA))
		--AKTUALIZACE STATISTICKYCH UDAJU HRACU (STATISTIKY(ID_HRA))
		RETURN NEW;
	END;
	$VYTVOR_TRIGGER_HRA_DOHRANA$ LANGUAGE plpgsql;	
	
CREATE TRIGGER HRA_DOHRANA AFTER UPDATE ON HRA EXECUTE PROCEDURE VYTVOR_TRIGGER_HRA_DOHRANA();

-- LAST TRIGGER - hlídání a aktualizace stavu hry, tj. zda nedospěla do remízy nebo vítězství jednoho z hráčů (volání funkcí REMIZA a VYHRA),
...
INSERT INTO HRA VALUES (3, 20, 20, 5, 2, '2020-02-24 20:45:17', 2, 0, 1, 0);
